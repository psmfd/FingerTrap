...
# Shell-originated methods have no api.ts counterpart by design: the Rust
# shell writes them into the sidecar's stdin (credentials/set per ADR-0022;
# shutdown per #167). They are void-returning notification handlers, which
# the Task-returning grep below deliberately does not count. A shell-originated
# method that returns Task would break this pairing — intentional friction
# because a response frame would be relayed to the WebView.
